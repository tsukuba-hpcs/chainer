...
def scatter_dataset_extended(dataset, comm, root=0, shuffle=False,
    ...
    to workers. To create a sub dataset, ``chainer.datasets.SubDatasetExtended`` is
    used.
    ...
    if comm.rank == root:
        ...
        for i in range(comm.size):
            ...
            if i == root:
                mine = chainer.datasets.SubDatasetExtended(dataset, b, e, order)
            else:
        ...
        assert data is not None
        (b, e) = data
        return chainer.datasets.SubDatasetExtended(dataset, b, e, order)
