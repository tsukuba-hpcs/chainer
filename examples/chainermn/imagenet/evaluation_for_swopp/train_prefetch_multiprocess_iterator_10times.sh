...
LOG_STDERR=${ROOT}/logs/prefetch_multiprocess_10times/${np}/${CURRENT_DATETIME}

mkdir -p ${ROOT}/logs/prefetch_multiprocess_10times/${np}

/usr/sbin/dropcaches 3

/system/apps/cuda/10.1/bin/nvprof \
    -o ${ROOT}/logs/prefetch_multiprocess_10times/${np}/${CURRENT_DATETIME}.%q{OMPI_COMM_WORLD_RANK}.nvvp \
